...
pub fn open_file(working_dir: PathBuf) -> Option<FileData> {
    ...
    let selected_file = match files.pick_file() {
        Some(selected_file_match) => selected_file_match,
        None => return None,
    };

    // set output variables
    let new_title_text = FileHandle::from(selected_file.clone()).file_name();
    //println!("the working directory is '{}'", new_title_text);
    let new_file_path = FileHandle::from(selected_file.clone());
...
pub fn open_dir() -> Option<PathBuf> {
    let folder_dialog = FileDialog::new()
        .set_title("open new folder");
    let new_folder = folder_dialog.pick_folder();
    println!("{:?}", new_folder);
    new_folder
}